    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .cors(cors -> cors.configurationSource(corsConfigurationSource())) // CORS(Cross-Origin Resource
                                                                                   // Sharing) 설정
                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS)) // 세션 정책을
                                                                                                              // STATELESS로
                                                                                                              // 설정 (세션
                                                                                                              // 사용 안함)
                ...
                .authorizeHttpRequests(authorize -> authorize // HTTP 요청에 대한 인가 규칙 설정
                        .requestMatchers("/api/auth/register", "/api/auth/login").permitAll() // 회원가입/로그인 API는 인증 없이 허용
                        .requestMatchers(HttpMethod.GET, "/api/trainers", "/api/trainers/*").permitAll() // 트레이너 정보
                                                                                                         // 조회(GET)는 인증
                                                                                                         // 없이 허용
                        .requestMatchers(HttpMethod.GET, "/api/products", "/api/products/*").permitAll() // 상품 정보
                                                                                                         // 조회(GET)는 인증
                                                                                                         // 없이 허용
                        .requestMatchers("/swagger-ui/**", "/v3/api-docs/**", "/actuator/health").permitAll() // API 문서
                                                                                                              // 및 헬스
                                                                                                              // 체크는 인증
                                                                                                              // 없이 허용
                        ...
                        .hasRole("TRAINER") // 트레이너 프로필 등록/수정은 TRAINER 역할만 가능
                ...
                .addFilterBefore(jwtAuthenticationFilter, UsernamePasswordAuthenticationFilter.class); // JWT 토큰 검증을 위한
                                                                                                       // 커스텀
                                                                                                       // 필터(JwtAuthenticationFilter)를
                                                                                                       // 로그인 필터 전에 추가
